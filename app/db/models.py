...
# ---------------------------------------------------------------------------
# Lab vocabulary enums  (UI-only — not mapped to any DB column)
# ---------------------------------------------------------------------------


class NonTreponemalTestType(str, enum.Enum):
    """Test-name choices for the non-treponemal lab editor row."""

    RPR = "RPR"
    VDRL = "VDRL"


class TreponemalTestType(str, enum.Enum):
    """Test-name choices for the treponemal confirmatory lab editor row."""
...
class NonTreponemalTiter(str, enum.Enum):
    """Valid titer values for RPR / VDRL results."""
...
class TreponemalTestResult(str, enum.Enum):
    """Result choices for treponemal confirmatory tests."""
...
class Case(Base):
    ...
    lot: Mapped[str | None] = mapped_column(
        String(10)
    )  # CDC stage code: 700=unknown, 710=primary, 720=secondary, 730=early non-primary non-secondary, 755=unknown duration or late
...
class CasePartnerRelationship(Base):
    ...
    def __repr__(self) -> str:
        return (
            f"<CasePartnerRelationship case={self.case_id} partner={self.partner_id}>"
        )
...
MAP_ITEMS: dict[int, dict] = {
    ...
    5: {
        ...
        "label": "Life Style (Hangouts/Social Activities)",
    },
    6: {"section": "Social History", "label": "E-Mail Address"},
    7: {
        "section": "Social History",
        "label": "Internet Activities/Info (Screen names, web sites)",
    },
    ...
    15: {
        ...
        "label": "General Medical (Medications, Illnesses, Hospitalizations)",
    },
    16: {
        "section": "Medical History",
        "label": "Patient's Understanding of Medical Aspects of Infection",
    },
    17: {
        "section": "Medical History",
        "label": "Illogical Aspects (Lesion, Lesion Hx, Lab Results)",
    },
    18: {
        "section": "Medical History",
        "label": "Drug Use (Concealed, Unclear, Downplayed)",
    },
    ...
    22: {
        "section": "Partners",
        "label": "Exposure Inconsistencies (Doesn't match what partners say)",
    },
    23: {"section": "Partners", "label": "No Partners Named During Lesion"},
    24: {
        "section": "Partners",
        "label": "Unexplained Change in Sexual Activity or Pattern",
    },
